...
| [#22021](https://github.com/stablyai/orca/issues/22021) Windows Sogou Shift-to-English drops the Electron TUI preedit         | Sogou reports Shift as Process/229 and/or key=Shift on ShiftLeft while composing, then an empty compositionend and a delayed insertText. Windows 229 suppression plus swallowing composing Shift as a bare modifier drop that commit. | Physical Shift (including Process/229 and composing key=Shift) reaches CompositionHelper. Idle Shift stays suppressed so kitty does not encode a bare modifier. An empty compositionend while that IME keydown still owes a commit waits for the held-key insertText instead of cancelling. Ordinary Windows letter-229 suppression stays. | Bypass policy lets ShiftLeft/ShiftRight through IME suppression and composing modifier keydowns; Process/229 and ordinary Shift while composing, then empty compositionend and delayed insertText, emit the latin once. |
