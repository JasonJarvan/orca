...
| [#22021](https://github.com/stablyai/orca/issues/22021) Windows Sogou Shift-to-English drops the Electron TUI preedit         | Sogou reports Shift as Process/229 and/or key=Shift on ShiftLeft, often after empty compositionend with isComposing already false, then a delayed insertText. Windows 229 suppression plus swallowing that Shift as a bare modifier drop that commit. | Every Shift keydown reaches CompositionHelper; that helper consumes it so kitty does not encode a bare modifier. Shift keyup stays suppressed. An empty compositionend while that IME keydown still owes a commit waits for the held-key insertText instead of cancelling. Ordinary Windows letter-229 suppression stays. | Bypass policy lets ShiftLeft/ShiftRight through IME suppression and never swallows Shift keydown as a modifier. Process/229 and ordinary Shift before or after empty compositionend, then delayed insertText, emit the latin once. |
