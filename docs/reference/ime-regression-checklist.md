...
| [#22021](https://github.com/stablyai/orca/issues/22021) Windows Sogou Shift-to-English drops the Electron TUI preedit         | Sogou reports Shift as Process/229 on ShiftLeft while composing, then an empty compositionend and a delayed insertText. Windows 229 suppression plus an empty-end flush drop that commit. | Physical Shift (including Process/229) reaches CompositionHelper. An empty compositionend while that IME keydown still owes a commit waits for the held-key insertText instead of cancelling. Ordinary Windows letter-229 suppression stays.                                                                                                                   | Bypass policy lets ShiftLeft/ShiftRight through during composition; Process/229 Shift with empty compositionend then delayed insertText emits the latin once, with and without the custom key handler.                                                                                                       |
